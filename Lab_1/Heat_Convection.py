...
with open('C:\\Users\\97254\\OneDrive\\Desktop\\Code\\TechnionCodeGit\\Bsc Winter Semseter 2026\\ChemEng-Ekronot-2H-Lab\\Lab_1\\temp_2D_hw.txt', 'r') as file:
...
def find_max_min_temp(temp_map):
    ...
    return f"{max_temp[0]:.2f} K, {min_temp[0]:.2f} K"
...
def create_heat_map(temp_map):
    ...
    plt.title('Section c - Temperature Distribution')
...
def create_heat_flux_maps(qx, qy):
    Lx, Ly = len(qx[0])-1, len(qx)-1 #dimenstions of the maps
    extent = 0, Lx, 0, Ly
    
    #heat flux in x direction
    plt.imshow(qx, cmap=plt.cm.plasma, interpolation='bicubic', extent=extent, origin='lower')
    plt.title('Section d - Distribution of the X Component of Heat Flux')
    plt.xlabel('x [cm]')
    plt.ylabel('y [cm]')
    plt.colorbar(cmap='plasma', label=r"$q''_x$ $[W/m^2]$")
 
    plt.show()

    #heat flux in y direction
    im2 = plt.imshow(qy, cmap=plt.cm.plasma, interpolation='bicubic', extent=extent, origin='lower')
    plt.title('Section d - Distribution of the Y Component of Heat Flux')
    plt.xlabel('x [cm]')
    plt.ylabel('y [cm]')
    plt.colorbar(cmap='plasma', label=r"$q''_y$ $[W/m^2]$")
